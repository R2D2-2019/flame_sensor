...
//                       hwlib::pin_oc &ir_led_5);
		...
		flame_sensor_c(hwlib::target::pin_adc &ir_led_1, hwlib::target::pin_adc &ir_led_2,
					   ...
					   hwlib::target::pin_adc &ir_led_5, unsigned int flame_threshhold, unsigned int threshhold_error,
					   unsigned int total_detection_angle);

		/**
		 * @brief calculate the average infrared level from the IR leds
		 *
		 * @return the average value from all the sensors with a given offset added to it
		 */
		 ...
		 */
		bool is_single_flame_detected(hwlib::target::pin_adc pin_to_check, unsigned int sensor_average);
